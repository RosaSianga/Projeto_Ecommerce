...
CREATE TABLE ECOMMERCE.PRODUTO(
		...
		CATEGORIA_ID INT NOT NULL REFERENCES ECOMMERCE.CATEGORIA(CATEGORIA_ID) ON DELETE CASCADE,
		...
		CONSTRAINT CHK_ESTOQUE CHECK (ESTOQUE_DISPONIVEL >= 0)
);
...
CREATE TABLE ECOMMERCE.ENDERECO_DE_ENTREGA(
		...
		CEP TEXT NOT NULL,
		FOREIGN KEY (CLIENTE_ID) REFERENCES ECOMMERCE.CLIENTE (CLIENTE_ID) ON DELETE CASCADE
);
...
CREATE TABLE ECOMMERCE.PEDIDO (
		...
		FOREIGN KEY (CLIENTE_ID) REFERENCES ECOMMERCE.CLIENTE (CLIENTE_ID) ON DELETE CASCADE,
		FOREIGN KEY (ENDERECO_ID) REFERENCES ECOMMERCE.ENDERECO_DE_ENTREGA (ENDERECO_ID) ON DELETE CASCADE,
...
CREATE TABLE ECOMMERCE.ITEM_DO_PEDIDO(
		...
		PEDIDO_ID INT NOT NULL REFERENCES ECOMMERCE.PEDIDO(PEDIDO_ID) ON DELETE CASCADE,
		PRODUTO_ID INT NOT NULL REFERENCES ECOMMERCE.PRODUTO(PRODUTO_ID) ON DELETE CASCADE,
...
CREATE TABLE ECOMMERCE.PAGAMENTO (
		...
		FOREIGN KEY (PEDIDO_ID) REFERENCES ECOMMERCE.PEDIDO(PEDIDO_ID) ON DELETE CASCADE,
		CONSTRAINT CHK_FORMA_PAGTO CHECK (FORMA_PAGAMENTO IN ('CARTÃO DE CRÉDITO', 'BOLETO', 'PIX', 'TRANSFERÊNCIA')),
		CONSTRAINT CHK_STATUS CHECK (STATUS IN ('APROVADO', 'RECUSADO', 'PENDENTE', 'ESTORNADO'))
);


/*
ALTER TABLE IF EXISTS ECOMMERCE.ENDERECO_DE_ENTREGA 
    ADD CONSTRAINT endereco_de_entrega_cliente_id_fkey FOREIGN KEY (cliente_id)
    REFERENCES ecommerce.cliente (cliente_id) MATCH SIMPLE
    ON UPDATE NO ACTION
    ON DELETE CASCADE;
...
ALTER TABLE IF EXISTS ecommerce.pedido
    ADD CONSTRAINT pedido_cliente_id_fkey FOREIGN KEY (cliente_id)
    REFERENCES ecommerce.cliente (cliente_id) MATCH SIMPLE
    ON UPDATE NO ACTION
    ON DELETE CASCADE;

ALTER TABLE IF EXISTS ecommerce.pagamento
    ADD CONSTRAINT pagamento_pedido_id_fkey FOREIGN KEY (pedido_id)
    REFERENCES ecommerce.pedido (pedido_id) MATCH SIMPLE
    ON UPDATE NO ACTION
    ON DELETE CASCADE;
